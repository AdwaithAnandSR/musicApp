...
const downloadAndSaveVideo = async (ytId, videoData, reqId) => {
    ...
    if (mongoose.connection.readyState === 1) {
        const existing = await musicModel.findOne({ $or: [{ ytId }, { title }] });
        if (existing) {
    ...
    const coverFile = files.find(f => f.startsWith(ytId) && !f.endsWith(".mp3") && !f.endsWith(".webm") && !f.endsWith(".m4a") && !f.endsWith(".part") && !f.endsWith(".ytdl") && !f.endsWith(".temp"));

    if (!audioFile) throw new Error(`Audio file not found for ${ytId} after download`);
    ...
    if (process.env.CLOUDINARY_URL || process.env.CLOUDINARY_CLOUD_NAME || process.env.CLOUDINARY_CLOUD_NAME_1) {
        ...
        audioUrl = await uploadToCloudinary(path.join(downloadDir, audioFile), "video", "musicApp/songs");
        if (coverFile) coverUrl = await uploadToCloudinary(path.join(downloadDir, coverFile), "image", "musicApp/covers");
    }

    if (mongoose.connection.readyState === 1) {
        console.log(`[SAVING] Storing metadata in MongoDB...`);
        await musicModel.create({ title, url: audioUrl, cover: coverUrl, duration, artist: uploader, ytId, stableRandom: Math.random() });
    }
...
const connectDB = async () => {
    ...
    const uri = process.env.MONGODB_URI || (pass ? `mongodb+srv://AdwaithAnandSR:${pass}@cluster0.8os2c.mongodb.net/${dbName}?retryWrites=true&w=majority&appName=Cluster0` : null);
    if (!uri) { console.warn("MONGODB_URI or MONGODB_PASS is not set."); return; }
    try {
    ...
    for (let i = 0; i < channels.length; i++) {
        ...
        for (let j = 0; j < videos.length; j++) {
            ...
            if (!isInitialSync && ytId === lastSongId) {
                console.log(`Found lastSongId (${lastSongId}). Boundary reached.`);
                break;
            }
            ...
            if (videoData.timestamp) {
                currentIsoTimestamp = new Date(videoData.timestamp * 1000).toISOString();
                ...
                currentIsoTimestamp = new Date(`${y}-${m}-${d}T00:00:00.000Z`).toISOString();
            } else {
                currentIsoTimestamp = new Date().toISOString();
            }

            if (!isInitialSync && lastSongTimestamp && currentIsoTimestamp <= lastSongTimestamp) {
                console.log(`Timestamp ${currentIsoTimestamp} is older or equal to lastSongTimestamp ${lastSongTimestamp}. Boundary reached.`);
                break;
            }

            newVideoRange.push({ ytId, videoData, currentIsoTimestamp });
            
            if (isInitialSync && newVideoRange.length >= INITIAL_CHANNEL_LIMIT) {
                console.log(`Initial sync limit reached (${INITIAL_CHANNEL_LIMIT}). Stopping range search.`);
                break;
            }
        }
        ...
        for (const item of newVideoRange) {
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    ...
                    console.error(`[ERROR] Attempt ${attempt}/3 failed for ${ytId}:`, e.message);
